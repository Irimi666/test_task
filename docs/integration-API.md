...
* **3.1. Сценарий 1: Реакция на события (Webhook)**

    Настроен вебхук для получения мгновенных уведомлений. Это позволяет отправлять пуш‑уведомления пользователю почти в реальном времени и наименьшей нагрузкой на сервер.

    * Зарегистрировать вебхук, сообщить сервису доставки URL вашего эндпоинта и тип событий, которые нужно отслеживать, пример:
    ...
    ```
    `event_type` - событие, которое будут отслеживаться через вебхук
...
* **3.2. Сценарий 2: Получение списка обновлений статусов за период** 
